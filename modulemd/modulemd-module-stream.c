                                             ...
                                             GError **error)
{
  ...
  buildtime_modules =
    modulemd_dependencies_get_buildtime_modules_as_strv (deps);
  ...
  for (guint i = 0; i < g_strv_length (buildtime_modules); i++)
    {
      streams = modulemd_dependencies_get_buildtime_streams_as_strv (deps, module);
          ...
          if (!expanded_deps)
            {
              ...
              modulemd_build_config_add_buildtime_requirement (new_dep, module, stream);
              ...
              for (guint k = 0; k < expanded_deps->len; k++)
                {
                  ...
                  new_dep = modulemd_build_config_copy (g_ptr_array_index (expanded_deps, k));
                  modulemd_build_config_add_buildtime_requirement (new_dep, module, stream);
                  g_ptr_array_add (new_expanded_deps, g_steal_pointer (&new_dep));
                }
            }
  ...
  /* repeat for each runtime module... */
  for (guint i = 0; i < g_strv_length (runtime_modules); i++)
    {
      streams = modulemd_dependencies_get_runtime_streams_as_strv (deps, module);
      /* for each runtime module stream... */
      for (guint j = 0; j < g_strv_length (streams); j++)
        {
          ...
          if (!expanded_deps)
            {
              new_dep = modulemd_build_config_new ();
              modulemd_build_config_add_runtime_requirement (new_dep, module, stream);
              g_ptr_array_add (new_expanded_deps, g_steal_pointer (&new_dep));
            }
          /* otherwise, expand on what we already have */
          else
                                             ...
                                             GError **error)
{
  ...
  g_autoptr (ModulemdDependencies) v2_deps = NULL;
  g_autoptr (GError) nested_error = NULL;
  g_autoptr (GPtrArray) expanded_deps = NULL;
  ...
  /* Iterate through the V2 dependencies */
  for (guint i = 0; i < v2_stream->dependencies->len; i++)
    {
      v2_deps =
        MODULEMD_DEPENDENCIES (g_ptr_array_index (v2_stream->dependencies, i));

      /* perform stream expansion on each of the V2 deps */
      expanded_deps =
        modulemd_module_stream_expand_v2_to_v3_deps (v2_deps, &nested_error);
      if (!expanded_deps)
        {
          g_propagate_prefixed_error (
            error,
            g_steal_pointer (&nested_error),
            "Unable to expand stream V2 dependencies to V3: ");
          return NULL;
        }

      /* create a V3 stream for each of the expanded V2 deps */
      for (guint j = 0; j < expanded_deps->len; j++)
        {
          ex_dep = (ModulemdBuildConfig *)g_ptr_array_index (expanded_deps, j);

          copy = modulemd_module_stream_v3_new (
            modulemd_module_stream_get_module_name (from),
            modulemd_module_stream_get_stream_name (from));

          /* copy in expanded platform, runtime_deps, buildtime_deps */
          modulemd_module_stream_v3_set_platform (
            copy, modulemd_build_config_get_platform (ex_dep));
          ...
          modules =
            modulemd_build_config_get_buildtime_modules_as_strv (ex_dep);
          for (guint i = 0; i < g_strv_length (modules); i++)
            {
          ...
          STREAM_UPGRADE_IF_SET_WITH_LOCALE (
            v2, v3, copy, v2_stream, description);
          ...
          if (!modulemd_module_index_add_module_stream (
                index, MODULEMD_MODULE_STREAM (copy), &nested_error))
            {
              g_propagate_error (error, g_steal_pointer (&nested_error));
              return NULL;
            }
      ...
      g_clear_pointer (&expanded_deps, g_object_unref);
    }
